...
$lx = 30;
$ly = 30;
$draw1->line($x_start,$y_start+$y*$scale+$ly,$x_start+$x*$scale/2-30,$y_start+$y*$scale+$ly);
$draw1->line($x_start+$x*$scale/2+30,$y_start+$y*$scale+$ly,$x_start+$x*$scale,$y_start+$y*$scale+$ly);
...
if($x_z){$z_out = $z_out_l." cm";;}
else {$z_out = '?';}

$draw3->annotation($x_l1_start+$dx*$l1+5-5 ,$y_l1_start+0  + $dy*$l1,$z_out );
...
$draw4->annotation($x_start+$x*$scale/2+20 ,$y_start+$y*$scale/2+12,"l");
$size = 18;
$draw4->setFontSize($size);
// $draw4->annotation($x_start+$x*$scale/2+70 ,$y_start+$y*$scale/2+12-20,"2");
