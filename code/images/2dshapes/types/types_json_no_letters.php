...
class types {
    ...
    function __construct($t){
        ...
        if (count($t)>=3){$canvas_width = 600; $canvas_height = 600;}
        // 2
        if (count($t)==2){$canvas_width = 600; $canvas_height = 300;}
        // 1
        if (count($t)==1){$canvas_width = 300; $canvas_height = 300;}

        $imagick->newImage($canvas_width,$canvas_height, new ImagickPixel('white')); //max (x)+10, max(y)+10
    ...
    function paralelogram($n){
        ...
        if ($this->positive_numbers<=3){
            ...
            $k = floor($n/2);
            $d->line(110+($j)*300,120+300*($i),   260+($j)*300,120+300*($i));
            $d->line(260+($j)*300,120+300*($i),  210+($j)*300,260+300*($i));
            $d->line(210+($j)*300,260+300*($i), 60+($j)*300,260+300*($i));
            $d->line(60+($j)*300,260+300*($i),  110+($j)*300,120+300*($i));
        }
    }
