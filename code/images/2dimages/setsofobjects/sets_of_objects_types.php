...
function scattered_objects($n_elem,$x_from,$x_to,$y_from,$y_to){
    ...
    do{
        ...
        for ($i=0;$i<count($arr);$i+=1){   
            if ((($x<$arr[$i]->x+$w)and($x>$arr[$i]->x-$w)and($y<$arr[$i]->y+$w)and($y>$arr[$i]->y-$w))){
                $flag = 0;
            } 
        }
        if ($flag){
    ...
    $arr = scattered_objects($n_elem1,20,250,20,200);
    if ($n_elem1<10){
        $w = 50;
    }else{
        $w = 500/$n_elem1;
...
for ($i = 0;$i<count($arr);$i+=1){
    imagecopyresampled($back,$objects[$type1],$arr[$i]->x, $arr[$i]->y, 0, 0, $w, $w, 64, 64);
}

$arr = scattered_objects($n_elem2,320,550,20,200);
...
for ($i = 0;$i<count($arr);$i+=1){
    imagecopyresampled($back,$objects[$type2],$arr[$i]->x, $arr[$i]->y, 0, 0, $w, $w, 64, 64);
}
