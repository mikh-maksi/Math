...
for ($i=0;$i<4;$i++){
    if($i==$right_place){
        array_push($arr_res,$right_n);
    }
    else{
    $deny = [];
    array_push($deny,$right_n);
...
imageline($back,0,250,225,250,$black );
// imageline($back,0,300,225,300,$black );
imageline($back,0,350,225,350,$black );
...
for ($i=0;$i<4;$i+=1){
imagefttext($back, 60, 0, 320, $y_line[$i],  $black, $font_file, $names[$i]);
}
