...
$right_place =$t;
$arr = array_slice($numbers, 0, 4);
$deny = [];
$arr_res = [];

for ($i=0;$i<4;$i++){
    if($i==$right_place){
        array_push($arr_res,$arr[$right_place]);
    }
    else{
    $deny = [];
    array_push($deny,$arr[$right_place]);
    array_push($deny,$arr[$i]);
    for($j=0;$j<count($arr_res);$j+=1)
    array_push($deny,$arr_res[$j]);

    do{
        $n = rand(1,10);
    }while(in_array($n,$deny));
    array_push($arr_res,$n);
}
}
...
$ob = $arr; //[4,2,5,1] random
$types = [0,1,2,3]; // [0,1,2,3] do not meter
$numbers = $arr_res; // $ob [4,2,5,1] random
$connections = [0,1,2,3]; //
...
for ($i=0;$i<4;$i+=1){
    ...
    imagefttext($back, 30, 0, 345, $y_line[$n]-5, $colors[$i], $font_file, $names[$i]);
    imagefttext($back, 60, 0, 375, $y_line[$i]+30,   $colors[$n], $font_file, $numbers[$i]);
}

for ($i=0;$i<4;$i+=1){
    // imagefttext($back, 30, 0, 345, $y_line[$i],  $black, $font_file, $names[$i]);
    // imagefttext($back, 60, 0, 320, 100,  $black, $font_file, "A");
}
