...
if (isset($_GET['angl'])){$angl = $_GET['angl'];}else{$angl=-1;}
...
$h = new Hexagone($draw,$l,$angl,$scale);
$draw = $h->out_print();

if ($scale !=-1){    
    $draw->setStrokeColor('#ff00ff');
    $h = new Hexagone($draw,$l,$angl,1);
    $draw = $h->out_print();    
    $draw->setStrokeColor('#000000');
}
...
class Hexagone{
    public $draw;
    function __construct($draw,$l,$angl,$scale){
        $this->draw = $draw;
        ...
        if ($l==0){$names = ['F','E','D','C','B','A'];}
        if ($l==-1) {$names = ['','','','','',''];}
        if ($l>0) {$names = [$l,$l,$l,$l,$l,$l];}

        if ($angl==0){$angles = ['a','b','c','d','e','f'];}
        if ($angl==-1) {$angles = ['','','','','',''];}
        ...
        $dx = cos(deg_rad(30))*$scale_base;
        $dy = sin(deg_rad(30))*$scale_base;

        $coords = [
            array("x"=>$x_start+$n*210, "y"=>$y_start,"name"=>$names[0],"angle"=>$angles[0]),
            array("x"=>$x_start-$dx+$n*210, "y"=>$y_start+$dy,"name"=>$names[1],"angle"=>$angles[1]),
            array("x"=>$x_start-$dx+$n*210, "y"=>$y_start+$dy+$scale_base,"name"=>$names[2],"angle"=>$angles[2]),
            array("x"=>$x_start+$n*210, "y"=>$y_start+$dy*2+$scale_base,"name"=>$names[3],"angle"=>$angles[3]),
            array("x"=>$x_start+$dx+$n*210, "y"=>$y_start+$dy+$scale_base,"name"=>$names[4],"angle"=>$angles[4]),
            array("x"=>$x_start+$dx+$n*210, "y"=>$y_start+$dy,"name"=>$names[5],"angle"=>$angles[5]),
    ];
